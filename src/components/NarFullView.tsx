...
interface NarFullViewProps {
  // En el dashboard ya hay un strip con título de tab, así que el header
  // interno se oculta. La página /alertas standalone lo muestra.
  hideTopHeader?: boolean;
}

export function NarFullView({ hideTopHeader = false }: NarFullViewProps = {}) {
  ...
  return (
    <>
      {/* HEADER — oculto cuando se renderiza dentro de un tab del dashboard */}
      {!hideTopHeader && (
      ...
      {/* Botón "Evaluar ahora" flotante cuando se oculta el header — para no perder la acción */}
      {hideTopHeader && (
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 8 }}>
          <button className="primary-btn" onClick={runEvaluate} style={{ fontSize: '0.82rem', padding: '6px 12px' }}>
            <Bell size={14} /> Evaluar ahora
          </button>
        </div>
      )}
