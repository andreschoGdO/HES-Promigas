...
export async function GET(_request: Request, { params }: RouteContext) {
  ...
  const { data, error } = await supabaseAdmin.from('purchase_order_items').select('*').eq('oc_id', id).order('posicion');
 ...
 * Reemplaza TODAS las líneas de detalle de la OC (mismo patrón que
 * consumables de inventario: se manda la lista completa, se borra lo
 * anterior y se inserta de nuevo — más simple que diffear).
 */
export async function PUT(request: Request, { params }: RouteContext) {
  try {
    ...
    const { error: delErr } = await supabaseAdmin.from('purchase_order_items').delete().eq('oc_id', id);
    ...
    const { data, error: insErr } = await supabaseAdmin.from('purchase_order_items').insert(rows).select('*').order('posicion');
